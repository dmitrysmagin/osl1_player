...
here. The reference implementation is `src/oldrld.c`
(`oldrld_generation()` returns `OLDRLD_GEN_ALB`). Where a field's meaning is
confirmed it is stated plainly; where it is inferred or unverified it is marked
...
`oldrld.c` parses none of it; a player that wanted to expose individual cues
would start here.
...
| Offset | Field      | Notes                                                    |
|--------|------------|----------------------------------------------------------|
| +0x00  | note       | 0 = no note; otherwise 17–96 observed. Note 12 is C-1 — the driver indexes its 96-entry note table with `note − 0x0C`. |
...
> field order, the 1-based selector and §4.2's volume rule. It then does
...
simply index past the array. Every other selector in the corpus is in range.
...
`src/opl_dev.c` is melodic-only, so `oldrld.c` reports the row width as
...
| +0x26  | u8       | rhythm      | 0 = melodic; 6 = bass drum, 7 = snare, 8 = tom, 9 = cymbal, 10 = hi-hat (§8). **Range-check it** — see below. |
| +0x27  | —        | —           | Unused tail.                                |

### The 13 editor fields

The order is fixed by the driver's own field table (the same table appears in
`ADLIB.DRV`, annotated at `../BSSJS_ADLIB.DRV.annotated.asm:1493`–`1722`):
...
everything else as melodic, which is what `src/oldrld.c` does.
...
There is no second instrument source to choose between, so `oldrld.c` forces
`editor` for `.ALB` regardless of the setting.
...
`0x0335`); `0x5D37` = 23863, and 1 193 182 / 23863 = **50.0016 Hz**. Turning
...
`0x0639`): one run once per row, one run every tick. `0x0647`/`0x0648` are the
bare-`ret` stubs.
...
The handler at `0x0766` is four instructions:
...
stop), reset the tick counter, store the speed. Byte-for-byte the same routine
...
The tick table above sends `0x0D` to a real handler at `0x073B` and `0x0E` to
...
`ADLIB.EXE` is a direct descendant of `BSSJS/ADLIB.DRV`, the `B4` driver: 100 of
359 sampled 32-byte windows of the older driver appear verbatim in it, the
`INT 60h` API is the same, and the 96-entry note table is byte-identical
(`0x0157 0x016C 0x0181 …` at `0x1203` here, `0x1392` there, with the same
`sub bl,0x0C` C-1 base). It is the *same driver, one generation on*, retargeted
at the exported container.
...
paragraph table at `+0x158` instead of `+0x118`, and produce garbage.
...
Two caveats on the find. The games themselves presumably still linked their own
...
game would be. And `ADLIB.EXE` v3.00 is not annotated in full the way
`BSSJS_ADLIB.DRV` is; the citations in this document are the routines that
matter to the container and the row reader.
...
`OLDMUSIC/` plus `LETHAL3/`. Every one parses cleanly under `src/oldrld.c`.
